# Generated by Powerlevel10k configuration wizard on 2024-07-11 at 14:35 CEST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH 
        ...
        # if local branch and tag
      local branch=${(V)VCS_STATUS_LOCAL_BRANCH}
      local tag=${(V)VCS_STATUS_TAG}
      res+="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${meta}[$fg[magenta]${branch//\%/%%}$reset_color|"
      res+="${meta}$fg[magenta]${tag//\%/%%}$reset_color]"
    fi
    ...
    [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&  # <-- this line
      res+="${meta}[${clean}$fg[magenta]:${VCS_STATUS_COMMIT[1,8]}$reset_color]"
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      ...
      (( VCS_STATUS_COMMITS_BEHIND )) && res+="${meta}⇣${VCS_STATUS_COMMITS_BEHIND}${clean}"
      # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
      # (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
      (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${meta}⇡${VCS_STATUS_COMMITS_AHEAD}${clean}"
    ...
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+="${meta}⇠${VCS_STATUS_PUSH_COMMITS_BEHIND}${clean}"
    # (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" "
    # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${meta}⇢${VCS_STATUS_PUSH_COMMITS_AHEAD}${clean}"
    # close the section
    (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND || VCS_STATUS_PUSH_COMMITS_AHEAD || VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+="|"
    # 'merge' if the repo is in an unusual state.
    [[ -n $VCS_STATUS_ACTION     ]] && res+="${conflicted}${VCS_STATUS_ACTION}"
    # ~42 if have merge conflicts.
    (( VCS_STATUS_NUM_CONFLICTED )) && res+="${conflicted}✖${VCS_STATUS_NUM_CONFLICTED}"
    # +42 if have staged changes.
    (( VCS_STATUS_NUM_STAGED     )) && res+="${conflicted}●${VCS_STATUS_NUM_STAGED}"
    # !42 if have unstaged changes.
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+="${clean}✚${VCS_STATUS_NUM_UNSTAGED}"
    # Display OK symbol if no change
    (( !VCS_STATUS_NUM_CONFLICTED )) && (( !VCS_STATUS_NUM_STAGED     )) && (( !VCS_STATUS_NUM_UNSTAGED   )) && res+="${clean}✔"
    # Add a space
    # res+=" "

    # *42 if have stashes.
    (( VCS_STATUS_NUM_UNTRACKED  || VCS_STATUS_STASHES        )) && res+="${meta}・${clean}"
    (( VCS_STATUS_STASHES        )) && res+="${clean}$fg[cyan]⚑${VCS_STATUS_STASHES}$reset_color${clean}"
    # ?42 if have untracked files. It's really a question mark, your font isn't broken.
    # See POWERLEVEL9K_VCS_UNTRACKED_ICON above if you want to use a different icon.
    # Remove the next line if you don't want to see untracked files at all.
    (( VCS_STATUS_NUM_UNTRACKED  )) && res+="${meta}${(g::)POWERLEVEL9K_VCS_UNTRACKED_ICON}${VCS_STATUS_NUM_UNTRACKED}${clean}"
